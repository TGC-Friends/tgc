...
# inits
httpPoolManger = urllib3.PoolManager()
...
def initial_formview(request):
	...
	if request.method == 'POST':
		...
		if formfields.is_valid():
			...
			'attendant': formfields.cleaned_data['attendant'],
			'additionalnotes': formfields.cleaned_data['additionalnotes'],
			}
			print(returnDict)

			# 1. create new user, if user exists, then 
		...
		# form not valid
			...
			'attendant': formfields.data['attendant'],
			'additionalnotes': formfields.data['additionalnotes'],
			}
			print(returnDict)
